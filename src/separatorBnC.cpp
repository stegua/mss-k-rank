...
separatorBnC(graph_t* g, graph_t* h, double* x_bar, GRBmodel* master, double timeout, int iter,
	...
	while (true) {
		...
		if (status == GRB_TIME_LIMIT) {
			...
			POST("set time", GRBsetdblparam(GRBgetenv(separa), GRB_DBL_PAR_TIMELIMIT, timeout - runtime));
		}
		else {
		...
		double rnt;
		POST("get time", GRBgetdblattr(separa, GRB_DBL_ATTR_RUNTIME, &rnt));
		runtime += rnt;
		mydata.runtime = timeout - runtime;
		if (timeout - runtime < 1e-09)
			break;
	}
	solCount = 0;
