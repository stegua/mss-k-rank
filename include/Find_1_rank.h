...
* @brief
*/
bool MyTestPair(vertex_t v, vertex_t h, const vertex_set_t& Rs, const BitGraph* G) {
   if (!G->is_edge(h, v))
      return false;
   return true;
}

void find_best_1_rank(vertex_set_t& Vs,
                      int n_vs,
                      double vs_cost,
                      vertex_set_t& Rs,
                      double rs_cost,
   ...
   // Naive pruning
   if (rs_cost + vs_cost <= std::max(1.0 + CUT_VIOL, best_cost))
      return;

   if (best_cost > 1.0 + CUT_VIOL) {
   ...
   Rs.push_back(head);
   find_best_1_rank(Vs, n_vs - n_ns, vs_cost - ns_cost, Rs, rs_cost + G->node_weight(head),
                    Best, best_cost, G, CUT_VIOL, nodes, start_time);
   Rs.pop_back();

   find_best_1_rank(Vs, n_vs - 1, vs_cost - G->node_weight(head), Rs, rs_cost,
                    Best, best_cost, G, CUT_VIOL, nodes, start_time);
}
...
best_1_rank(const BitGraph* G, double CUT_VIOL, tpoint start_time) {
   vertex_set_t Vs;
   ...
   find_best_1_rank(Vs, Vs.size(), vs_cost, Rs, 0.0, Best, best_cost, G, CUT_VIOL, nodes, start_time);
